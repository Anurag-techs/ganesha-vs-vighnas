...
class AudioManager {
  constructor() {
    this.ctx = null;
    this.soundEnabled = this.loadState('ganesha_sound_enabled', true);
    this.musicEnabled = this.loadState('ganesha_music_enabled', true);
    this.bgmTimer = null;
    this.bgmNoteIndex = 0;
  }
  ...
  toggleSound() {
    ...
    this.saveState('ganesha_sound_enabled', this.soundEnabled);
    this.emitAudioState();
    if (this.soundEnabled) this.playButtonClickSound();
  }

  toggleMusic() {
    this.musicEnabled = !this.musicEnabled;
    this.saveState('ganesha_music_enabled', this.musicEnabled);
  ...
  // === PROCEDURAL FESTIVE SYNTH BGM LOOP ===

  startBGM() {
    this.initContext();
    if (!this.musicEnabled || !this.ctx || this.bgmTimer) return;

    // Festive Indian pentatonic scale frequencies (Raga Mohanam / Bhupali: C D E G A C)
    ...
    this.bgmTimer = setInterval(() => {
      ...
      const freq = bgmNotes[this.bgmNoteIndex % bgmNotes.length];
      this.bgmNoteIndex++;
  ...
  playModakSound() {
    ...
    osc.frequency.exponentialRampToValueAtTime(1174.66, this.ctx.currentTime + 0.15);
    gain.gain.setValueAtTime(0.35, this.ctx.currentTime);
    gain.gain.exponentialRampToValueAtTime(0.01, this.ctx.currentTime + 0.2);
    osc.connect(gain);
    gain.connect(this.ctx.destination);
